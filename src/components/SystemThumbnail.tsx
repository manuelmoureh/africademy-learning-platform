  ...
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg p-2.5 space-y-2 shadow-lg shadow-black/10">
      <div className="flex items-center gap-1.5 pb-1.5 border-b border-dashed border-[#12102A]/10">
        ...
        <span className="text-[8px] font-bold font-mono text-[#12102A]/60">{label}</span>
      </div>
  ...
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg p-3 space-y-1.5 shadow-lg shadow-black/10">
      <div className="flex items-center gap-1.5 pb-1.5 border-b border-dashed border-[#12102A]/10">
        ...
        <p className="text-[9px] font-bold font-mono text-[#12102A]">{lines[0]}</p>
      </div>
      {lines.slice(1, -1).map((l) => (
        <p key={l} className="text-[8px] font-mono text-[#12102A]/50">{l}</p>
      ))}
      <p className="text-[9px] font-black font-mono text-[#12102A] pt-1 border-t border-dashed border-[#12102A]/10">{lines[lines.length - 1]}</p>
    </div>
  );
}
...
function CardListMockup({ lines, tone = 'positive' }: { lines: string[]; tone?: 'positive' | 'warning' }) {
  ...
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg px-3 py-2.5 flex items-center gap-2 shadow-lg shadow-black/10">
      ...
      <span className="text-[9px] font-bold font-mono text-[#12102A] flex-1">{lines[0]}</span>
      <span
        className={`shrink-0 text-[8px] font-bold font-mono px-1.5 py-0.5 rounded-full flex items-center gap-0.5 ${
  ...
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg p-2.5 space-y-1.5 shadow-lg shadow-black/10">
      <div className="flex items-center gap-1.5 pb-1.5 border-b border-dashed border-[#12102A]/10">
        ...
        <p className="text-[9px] font-bold font-mono text-[#12102A]">{label}</p>
      </div>
      <div className="grid grid-cols-7 gap-1">
        {days.map((_, i) => (
          <div
            key={i}
            className={`aspect-square rounded flex items-center justify-center text-[8px] font-bold font-mono ${
  ...
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg overflow-hidden shadow-lg shadow-black/10">
      <div className="flex items-center gap-1.5 px-2.5 pt-2.5">
        ...
        <span className="text-[7px] font-bold font-mono text-[#12102A]/70">{label}</span>
      </div>
      ...
      <div className="p-2.5 space-y-1">
        ...
        <div className="flex items-center justify-between">
          <p className="text-[7px] font-mono text-[#12102A]/50">{lines[1]}</p>
