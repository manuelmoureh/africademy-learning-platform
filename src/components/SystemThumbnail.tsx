...
const CONFIG: Record<string, ThumbnailConfig> = {
  'whatsapp-retail-agent': { kind: 'chat', lines: ['Do you have size 40?', 'Yes! KES 2,800, in stock ✅'] },
  ...
  'booking-scheduler-agent': { kind: 'calendar', lines: [] },
  'social-content-agent': { kind: 'post', lines: ['New stock just arrived', 'Tap to shop the drop'] },
  ...
  'food-ordering-agent': { kind: 'chat', lines: ['2x Chapati, 1x Chicken Stew', 'Order confirmed, 25 min ETA'] },
};
...
function DocumentMockup({ lines }: { lines: string[] }) {
  return (
    <div className="w-full max-w-[160px] bg-white/95 rounded-lg p-3 space-y-1.5">
      <p className="text-[9px] font-bold text-[#12102A]">{lines[0]}</p>
      {lines.slice(1, -1).map((l) => (
        <p key={l} className="text-[8px] text-[#12102A]/50 border-b border-[#12102A]/5 pb-1">{l}</p>
      ))}
      <p className="text-[9px] font-black text-[#12102A] pt-0.5">{lines[lines.length - 1]}</p>
    </div>
  );
}
...
function CardListMockup({ lines }: { lines: string[] }) {
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg px-3 py-2.5 flex items-center justify-between gap-2">
      <span className="text-[9px] font-bold text-[#12102A] truncate">{lines[0]}</span>
...
function CalendarMockup() {
  ...
  return (
    <div className="grid grid-cols-7 gap-1 w-full max-w-[170px]">
      {days.map((_, i) => (
        <div
          key={i}
...
function PostMockup({ lines }: { lines: string[] }) {
  return (
    <div className="w-full max-w-[170px] bg-white/95 rounded-lg overflow-hidden">
      <div className="h-10 bg-[#F5A623]/25" />
...
export const SystemThumbnail: React.FC<{ trackId: string }> = ({ trackId }) => {
  ...
  switch (config.kind) {
    case 'chat': return <ChatMockup lines={config.lines} />;
    ...
    case 'post': return <PostMockup lines={config.lines} />;
    default: return null;
  }
};
